...
import org.ndshop.user.common.utils.email.Email;
import org.ndshop.user.common.utils.email.EmailUtil;
...
public class SendEmail {
    ...
    public void sendEmail() throws Exception {
        ...
        email.setUsername("xinaml@qq.com");//登录账号（可读配置，不用设置 ）
        email.setPassword(new BASE64Encoder().encode("123456789".getBytes()));//登录密码（可读配置，不用设置 ）
        email.setSenderName("小明..."); //发送人名字（可读配置，不用设置 ）
        email.setHost("smtp.163.com"); //发送人名字（可读配置，不用设置 ）
